...
float VEL_JOGADOR = 3;		//velocidade dos jogadores

//BOLA:
float VEL_MAX_X = 1.5;		//velocidade horizontal maxima das bolas
...
void atualizaBolas(Bola bolas[], Jogador *p1, Jogador *p2, int bolasjogo) {
    ...
    for (int i = 0; i < 20; i++)
    {
		if(bolas[i].dentro == 1) 
		{
			...
			if(p1->p == 1)	//se a tecla para rebater do Jogador 1 estiver pressionada
				if(bolas[i].x >= p1->x && bolas[i].x <= p1->x + p1->w)	//se a bola esta dentro dos limites de largura do Jogador 1
					if(bolas[i].y + bolas[i].r >= p1->y && bolas[i].y - bolas[i].r <= p1->y + p1->h) //se a bola esta dentro dos limites de altura do Jogador 1
						if(bolas[i].vel_y > 0)	//se a bola estiver vindo de cima
							bolas[i].vel_y = -bolas[i].vel_y;
			
			//se a bola esta dentro do Jogador 2
			if(p2->p == 1)	//se a tecla para rebater do Jogador 2 estiver pressionada
				if(bolas[i].x >= p2->x && bolas[i].x <= p2->x + p2->w)	//se a bola esta dentro dos limites de largura do Jogador 2
					if(bolas[i].y + bolas[i].r >= p2->y && bolas[i].y - bolas[i].r <= p2->y + p2->h) //se a bola esta dentro dos limites de altura do Jogador 2
						if(bolas[i].vel_y < 0)	//se a bola estiver vindo de baixo
							bolas[i].vel_y = -bolas[i].vel_y;
		}
    }
	p2->p = 0;	//O jogador para de rebater
	p1->p = 0;	//O jogador para de rebater
}

//Lança as bolas
void canhao(Bola bolas[], int bolasjogo, int posicao, int atirou) {
	int i = 0;
	atirou = 0;
	for(int i = 0; i < 20; i++)
		if(bolas[i].dentro == 0)
		{
			...
			bolas[i].cor = al_map_rgb(200, 200, 200);	//adiciona cor a bola
			bolas[i].r = randfloat(4, 7);				//cria um raio para a bola
			bolasjogo++;								//aumenta a quantidade de bolas no jogo

			printf("Bola lancada no canhao %d\n\n", posicao);
...
int main(int argc, char **argv){
	...
	int tempod = 0;		//variavel para armazenar o tempo depois do jogo acabar
	int atirou = 0;		//variavel para armazenar se o canhao atirou ou nao

	//historico
	int who; 			//quem ganhou (se who = 1, Jogador 1 (RED) ganhou / se who = 2, Jogador 2 (BLUE) ganhou)
	int winp1, winp2;	//variavel para armazenar o numero de vitorias de cada jogador
	char h1[10], h2[10];
    ...
    {
        fgets(linha, 10, arq);
		winp1 =  atoi(strtok(linha, " "));
		winp2 = atoi(strtok(NULL, " "));
	}
	fclose(arq);
		...
		if(ev.type == ALLEGRO_EVENT_TIMER) {
			if(p1.pnt < 10 && p2.pnt < 10)
			{
				...
				if(al_get_timer_count(timer)%(int)FPS == 0) {
					...
					if(tempo % TEMP_BOLAS == 0)
					{
						canhao(bolas, bolasjogo, posicao, atirou);
						if(posicao == 3)
							posicao = 0;
						else
							posicao++;			
			...
			else
			{
				...
				//se passou 10 segundos depois do jogo acabar ele é fechado
				if(tempod == 10)
					playing = 0;

				al_clear_to_color(al_map_rgb(200, 200, 200));	//colore a tela de branco

				//textos
				al_draw_text(size_32, al_map_rgb(150, 0, 0), SCREEN_W/2+15, SCREEN_H/2, 0, pntp1);			//escreve o char com a pontuacao do jogador na tela
				al_draw_text(size_32, al_map_rgb(20, 20, 20), SCREEN_W/2, SCREEN_H/2, 1, "x");			//excreve um x entre a pontuacao de cada jogador
				al_draw_text(size_32, al_map_rgb(0, 0, 150), SCREEN_W/2-15, SCREEN_H/2, 2, pntp2);			//escreve o char com a pontuacao do jogador na tela
				al_draw_text(size_32, al_map_rgb(20, 20, 20), SCREEN_W/2, SCREEN_H/2 - 60, 0, " win");	//excreve quem ganhou

				al_draw_text(size_32, al_map_rgb(150, 0, 0), SCREEN_W/2, SCREEN_H/2 + 80, 2, "RED WINS: ");	//excreve "RED WINS"
				al_draw_text(size_32, al_map_rgb(0, 0, 150), SCREEN_W/2, SCREEN_H/2 + 40, 2, "BLUE WINS: ");//excreve "BLUE WINS"

				//se p1 ganhou
				if(p1.pnt > p2.pnt)
				{
					al_draw_text(size_32, al_map_rgb(150, 0, 0), SCREEN_W/2, SCREEN_H/2 - 60, 2, "RED");	//excreve "RED"
					who = 1;
					sprintf(h1, "%d", winp1+1);
					sprintf(h2, "%d", winp2);
					
				}

				//se p2 ganhou
				else if(p2.pnt > p1.pnt)
				{
					al_draw_text(size_32, al_map_rgb(0, 0, 150), SCREEN_W/2, SCREEN_H/2 - 60, 2, "BLUE");	//excreve "BLUE"
					who = 2;
					sprintf(h1, "%d", winp1);
					sprintf(h2, "%d", winp2+1);
				}
				al_draw_text(size_32, al_map_rgb(150, 0, 0), SCREEN_W/2, SCREEN_H/2 + 80, 0, h1);	//excreve o historico do Jogador 1 (RED)
				al_draw_text(size_32, al_map_rgb(0, 0, 150), SCREEN_W/2, SCREEN_H/2 + 40, 0, h2);	//excreve o historico do Jogador 2 (BLUE)
			}	
