	...
	while(playing) {
		...
		if(ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if(al_get_timer_count(timer)%(int)FPS == 0) {
                ...
                //a cada TEMP_BOLAS segundos uma bola é lançada de um canhão se houver menos de 20 bolas no jogo
